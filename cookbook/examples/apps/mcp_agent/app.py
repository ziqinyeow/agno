...
async def main() -> None:
    ...
    ####################################################################
    try:
        logger.info(f"Initializing MCPTools for {mcp_server_config.id}")
        ...
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                logger.info(f"MCP Tools List: {mcp_tools}")
                if (
                ...
                if len(agent_runs) > 0:
                    ...
                    for _run in agent_runs:
                        ...
                        if _run.response is not None:
                            add_message(
                                "assistant", _run.response.content, _run.response.tools
                            )
                else:
                    ...
                    with st.chat_message("assistant"):
                        ...
                        with st.spinner(":thinking_face: Thinking..."):
                            ...
                            try:
                                ...
                                add_message(
                                    "assistant", response, mcp_agent.run_response.tools
                                )
                            except Exception as e:
                                logger.error(
                                    f"Error during agent run: {str(e)}", exc_info=True
                                )
                                error_message = (
